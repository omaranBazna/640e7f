...
import { ChevronLeftIcon, ChevronRightIcon, CurrencyBangladeshiIcon } from "@heroicons/react/outline";
...
function Pagination({
  ...
  function pageCountFunction(pages){
    let floor=Math.floor(totalCount/pages)
    if(Math.abs(floor-totalCount/pages)>0){
      return floor+1;
    }else{
      return floor;
    }
   ...
   const pageCount=pageCountFunction(pageSize)
    ...
    >
      ...
      <li className="paginationItem">
        <button
          ...
          onClick={onNext}
          disabled={currentPage == pageCount} // change this line to disable a button.
        >
          <ChevronRightIcon />
        </button>
      </li>
